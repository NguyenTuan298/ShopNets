class ProductManager {
  ...
  init() {
    ...
    if (addCategory) {
      addCategory.addEventListener("change", () =>
        this.renderSpecifications(
          "product",
          addCategory.options[addCategory.selectedIndex]?.text || "",
          {}
        )
      );
    }
    ...
    if (editCategory) {
      editCategory.addEventListener("change", () =>
        this.renderSpecifications(
          "editProduct",
          editCategory.options[editCategory.selectedIndex]?.text || "",
          {}
        )
      );
    }
  ...
  populateEditForm(productData) {
    ...
    if (shortDescription)
      shortDescription.value = productData.short_description || "";
    ...
    if (productData.image && imagePreview) {
      ...
      imagePreview.innerHTML =
        '<div style="margin-top: 10px; color: #999; font-size: 12px;">No current image</div>';
    }
  ...
  async handleEditSubmit(e) {
    ...
    try {
      ...
      console.error("Error:", error);
      this.showErrors("editErrorMessages", [
...
ProductManager.prototype.renderSpecifications = function (
  formPrefix,
  categoryName,
  existingSpecs
) {
  const containerId =
    formPrefix === "product"
      ...
      : formPrefix + "SpecificationsContainer";
  const inputId =
    formPrefix === "product"
      ? "addSpecificationsInput"
      : formPrefix + "SpecificationsInput";
  ...
  template.forEach((key) => {
    ...
    row.innerHTML = `
            ...
            <input type="text" class="spec-input" data-spec-key="${key}" value="${
      existingSpecs[key] ? existingSpecs[key] : ""
  ...
  if (template.length === 0) {
    ...
    if (existingKeys.length > 0) {
      existingKeys.forEach((key) => {
        ...
        row.style.marginBottom = "8px";
        row.innerHTML = `
                    <input type="text" class="spec-key" placeholder="Tên thông số" style="flex:1;padding:8px;border:1px solid #ddd;border-radius:4px;" value="c">
